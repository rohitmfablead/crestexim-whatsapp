...
export const ContactSelectionCard: React.FC<ContactSelectionCardProps> = ({
  ...
  const filteredContacts = useMemo(() => {
    // Optimization: collect all member IDs in selected groups once for O(1) lookup
    const selectedGroupMemberIds = new Set<string>();
    if (filterGroups.length > 0) {
      filterGroups.forEach((groupId) => {
        const group = groups.find((g) => g.id === groupId);
        group?.members?.forEach((member) => selectedGroupMemberIds.add(member.id));
      });
    }
    ...
    return contacts.filter((contact) => {
      ...
      const matchesGroups = selectedGroupMemberIds.has(contact.id);
