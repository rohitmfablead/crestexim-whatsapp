...
const specificTemplateNames = [
  ...
  "new_business_conclave__fashion_show_poster",
  "new_business_conclave__fashion_show_text",
  "festival_offer",
  "customer_engagement__reminder",
  "seasonal_offer",
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (specificTemplateNames.includes(selectedTemplate.name)) {
        // For specific templates, automatically push "redirectwhatsapp" for the first button,
        // "redirectwhatsapp1" for the second, etc.
        formattedButtonVariables = buttonVariables.map((_, index) =>
          index === 0 ? "redirectwhatsapp" : `redirectwhatsapp${index}`
        );
      } else {
